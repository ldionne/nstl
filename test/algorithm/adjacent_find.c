...
NSTL_INSTANTIATE(NSTL_ADJACENT_FIND_CMP(nstl_pint, nstl_int, Compare))

#define adjacent_find nstl_adjacent_find(nstl_pint)
#define adjacent_find_cmp(first, last)                                         \
    nstl_adjacent_find_cmp(nstl_pint, Compare)(                                \
...
static void test_should_not_find_any_when_none_equal_and_adjacent(void) {
    ...
    nstl_pint found_cmp = adjacent_find_cmp(array, array + 10);
    assert_true(found == found_cmp);
